...
public class Picture extends SimplePicture {
     ...
     *
     * @return a copy of the current picture with the red decreased by half
     */
    ...
    /**
     * Method to decrease red by the passed percent
     *
    ...
    /**
     * Method to clear the blue from the picture (set the blue to 0 for all
     * pixels)
     */
    ...
    /**
     * Method to make a picture look like it was taken at sunset by reducing the
     * blue and green to make it look more red
     */
    ...
    /**
     * Method to change the color of each pixel in the picture object by passed
     * in amounts.
     *
    ...
    /**
     * Method to negate the picture using a nested loop
     */
    ...
    /**
     * Method to negate the pixels in the current picture
     */
    public void negate() {
        Pixel[] pixelArray = this.getPixels();
        Pixel pixel = null;
    ...
    /**
     * Method to lighten the colors in the picture if the distance to the passed
     * color is within the passed distance
     *
    ...
    /**
     * Method to mirror around a vertical line at the end of the picture based
     * on the width
     */
    ...
    /**
     * Method to mirror around a horizontal line at the end of the picture based
     * on the height
     */
    ...
    /**
     * Method to mirror around a horiztonal line in the middle based on the
    ...
    /**
     * Method to mirror part of the picture around a horiztonal mirror
     *
    ...
    /**
     * Method to mirror part of a picture around a vertical mirror point
     */
    ...
    /**
     * Method to blend two pictures horizontally with 10% of first, 80 percent
     * overlap of first and second, and last 10 percent of 2nd
     */
    ...
    /**
     * Method to blend two pictures horizontally with 20% of first, 60% overlap
     * of first and second, and last 20% of 2nd
     */
    ...
    /**
     * Method to copy the picture of a robot to the 100,100 of the current
     * picture
     */
...
//    public void crop(Picture pictObj, int X, int Y,int xMax,int yMax) {
//        Picture sourcePicture = pictObj;
//        Pixel sourcePixel = null;
//        Pixel targetPixel = null;
//        int sx = X;
//        int sy = Y;
//        // loop through the columns
//        for (int sourceX = sx, targetX = 100;
//                sourceX < xMax; sourceX++, targetX++) {
//            // loop through the rows
//            for (int sourceY = sy, targetY = 100;
//                    sourceY < yMax; sourceY++, targetY++) {
//                // set the target pixel color to the source pixel color
//                sourcePixel = sourcePicture.getPixel(sourceX, sourceY);
//                targetPixel = this.getPixel(targetX, targetY);
//                targetPixel.setColor(sourcePixel.getColor());
...
public void crop(Picture pictObj, int X, int Y,int xMax,int yMax)
{
  ...
  // loop through the columns
  for (int sourceX = X, targetX = 100;
       sourceX < xMax; sourceX++, targetX++)

  {
    // loop through the rows
    for (int sourceY = Y, targetY = 100;
         sourceY < yMax; sourceY++, targetY++)
    {
      // set the target pixel color to the source pixel color
      sourcePixel = sourcePicture.getPixel(sourceX,sourceY);
    ...
    /**
     * Method to copy flower pictures to create a collage. All the flower
     * pictures will be lined up near the bottom of the current picture (5
     ...
     * picture object starting with the left corner given by xStart. It will put
     * the sourcePicture at 5 pixels from the bottom of this picture
     *
...
//             300,0);
    ...
    /**
     * Method to copy from the passed source picture to the current picture
     * object The copying will start at startX, startY, and end at endX-1 and
     * endY-1 The copy will be placed starting at targetStartX, targetStartY
     *
    ...
    /**
     * Method that will copy all of the passed source picture into the current
     * picture object starting with the left corner given by xStart, yStart
     *
     * @param sourcePicture the picture object to copy
    ...
    /**
     * Method to simulate fog by chaging all pixels that are within the passed
     * dist to white to a variation on the passed color
     *
    ...
    /**
     * Method to change all pixels within dist to black to green
     *
     * @param dist if a color is less than this distance from black change it
     */
    ...
    /**
     * Method to lighten all pixels less than the passed distance from black
     *
     * @param dist if the distance from the current pixel to black is less than
     * this amount change it
     */
     ...
     *
     * @param dist if a pixel is less than this distance from white then darken
     * it
     */
    ...
    /**
     * Method to copy the picture of Katie but rotate her left 90 degrees on the
     * current picture
     */
    ...
    /**
     * Method to copy the flower but smaller (half as big) on to the current
     * picture
     */
    ...
    /**
     * Method to copy the picture of Jakita but smaller (half as big) to the
     * current picture
     */
    ...
    /**
     * Method to flip the picture so that the pixels that were on the right will
     * be on the left (this is like what you see in a mirror)
     *
    ...
    /**
     * Method to copy a flower but scaled to 2x normal size onto the current
     * picture
     */
    ...
    /**
     * Method to turn brown to red in a rectangular area specifed by startX,
     * endX-1, startY, endY-1
     *
     * @param startX the starting location to check in x
     * @param endX the last pixel checked is one less than this in x
     * @param startY the starting location to check in y
     * @param endY the last pixel checked is one less than this in y
     */
     ...
     * @param startY the starting location to check in y
     * @param endY the last pixel checked is one less than this in y
     * @param distance if the current color is within this distance to brown
     * then change it
     */
    public void turnBrownToRedInRectangle(int startX, int endX,
            int startY, int endY,
    ...
    /**
     * Method to double the red in a picture in a rectangular area if it is less
     * than the passed distance to the passed color
     *
     * @param startX the x value at the top left of the rectangular area
     * @param startY the y value at the top left of the rectangular area
     ...
     * @param compareColor the color to compare the current pixel color to
     */
    ...
    /**
     * Method to change all the pixels within a passed distance of a passed
     * color in a range specified by a startX, startY, endX, and endY by passed
     * red, green, and blue factors.
     *
     * @param compareColor the color to compare to
     * @param distance the distance that this should be within (less than)
     * @param redFactor the amount to multiply the current red value by
     * @param greenFactor the amount to multiply the current green value by
     * @param blueFactor the amount to multiply the current blue value by
     * @param startX the starting x value to use in checking the pixels
     * @param startY the starting y value to use in checking the pixels
     * @param endX the ending x value to use in checking the pixels (< this)
   * @
     * param endY the ending y value to use in checking the pixels (< this)
     */
    ...
    /**
     * Method to remove red eye from the current picture object in the rectangle
     * defined by startX, startY, endX, endY. The red will be replaced with the
     * passed newColor
     *
    ...
    /**
     * Method to change the current picture to a sepia tint (modify the middle
     * colors to a light brown and the light colors to a light yellow and make
     * the shadows darker)
     */
    ...
    /**
     * Modify the current picture based on the distance to the final picture,
     * the current index, and the total distance
     *
    ...
    /**
     * Method to change a range in the current picture to a sepia tint (modify
     * the middle colors to a light brown and the light colors to a light yellow
    ...
    /**
     * Method to posterize (reduce the number of colors) in the picture. The
     * number of reds, greens, and blues will be 4
     */
     ...
     * have a color distance less than the passed amount to white or black with
     * the passed replacement color
     *
    ...
    /**
     * Method to create a new picture that is scaled up by the passed number of
     * times.
     *
     ...
     *
     * @param numPixels the number of pixels to average in all directions. So if
     * the numPixels is 2 then we will average all pixels in the rectangle
    ...
    /**
     * Method to do a simple edge detection by comparing the absolute value of
     * the difference between the color intensities (average of the color
     * values) between a pixel and the pixel below it. If the absolute value of
     * the difference between the color intensities is less than a passed amount
     * the top pixel color will be set to white. Otherwise it is set to black.
     *
     * @param amount if the absolute value of the differences in the color
     * average is less than this set the color to white, else black
     */
    ...
    /**
     * Method to check for the edge by comparing the current pixel to the one on
     * the right
     *
     * @param amount the amount to use to see if it is an edge
     */
    ...
    /**
     * Method to check for an edge using the average of the pixel to the right
     * and below
     *
    ...
    /**
     * Method to do a simple edge detection in a range by comparing the absolute
     * value of the difference between the color intensities (average of the
     * color values) between a pixel and the pixel below it. If the absolute
     * value of the difference between the color intensities is less than a
     * passed amount the top pixel color will be set to white. Otherwise it is
     * set to black.
     *
     * @param amount if the absolute value of the differences in the color
     * average is less than this set the color to white, else black
    ...
    /**
     * Method to replace the background in the current picture with the
     * background from another picture
     *
     ...
     * @param oldBackground the old background to replace
     * @param newBackground the new background to use
     * @param threshold if the distance between the current pixel color and the
     * background pixel color is less than this amount use the new background
     * pixel color
     */
    public void swapBackground(Picture oldBackground,
            Picture newBackground,
    ...
    /**
     * Method to do chromakey using the passed background color and the distance
     * to the color
     *
     * @param newBg the new background image to use to replace
     * @param color the background color to compare to
     * @param dist the distance that limits the chromakey it will happen if the
     * distance is less than or equal to this value
     */
     ...
     *
     * @param upperX the top left corner x of the enclosing rectangle
     * @param upperY the top left corner y of the enclosing rectangle
     ...
     * @param endX the x value of the end of the speech triangle
     * @param endY the y value of the end of the speech triangle
     */
     ...
     *
     * @param newBg the new background image to use to replace the blue from the
     * current picture
     */
     ...
     *
     * @param newBg the new background image to use to replace the green from
     * the current picture
     */
    ...
    /**
     * Method to change the current colors to just green, red, and blue. If the
     * average at a pixel is less than 85 set the color to green, if it is less
     * than 170 set it to red, else set it to blue
     */
    ...
    /**
     * Method to do chromakey using a blue background in a range
     *
     * @param newBg the new background image to use to replace the blue from the
     * current picture
     ...
     *
     * @param newBg the new background image to use to replace the red from the
     * current picture
     */
    ...
    /**
     * Method to set the blue values to 100 in a rectangle of 100 pixels with x
     * and y starting at 0 and ending at 99
     */
    ...
    /**
     * Method to set the blue to zero if the distance from this color to blue is
     * greater than 100
     */
    ...
    /**
     * Method to set the blue to zero if the current blue value is greater than
     * 100
     */
    public void setBlue() {
        Pixel[] pixelArray = getPixels();
    ...
    /**
     * Method to show different drawing capabilities. It will draw a string, a
     * line, a filled rectangle, the outline of a rectangle, the outline of an
     * oval, and a filled arc.
     */
     ...
     * Method to draw a face to demonstrate drawing ovals and arcs
     *
     * @param upperLeftX the upper left x of the enclosing rectangle
     * @param upperLeftY the upper left y of the enclosing rectangle
    ...
    /**
     * Method to draw a rainbow using arcs to draw the red, orange, yellow,
     * green, blue, purple and white bands.
     */
    ...
    /**
     * Method to draw a horizontally centered string at the top of the picture
     *
     * @param text the string to draw
     */
    ...
    /**
     * Method to draw a picture with a succession of filled rectangles with the
     * top left corner the darkest and the bottom right the lightest on the
     * current picture
     */
    ...
    /**
     * Method to draw a picture with a succession of rectangles on the current
     * picture
     */
    ...
    /**
     * Method to add two crossed lines to a picture. One line will go from the
     * top left corner to the bottom right corner. The other will go from the
    ...
    /**
     * Method to draw a dashed X on the current picture
     *
     * @param color the color to draw the X in
    ...
    /**
     * Method to create a new picture by rotating the current picture by the
     * given degrees
     *
    ...
    /**
     * Method to add text rotated to the right 90 degrees
     *
    ...
    /**
     * Method to create a new picture by shearing the current picture by the
     * given x and y factors
     *
     * @param xFactor multiplier to use to shift in x direction based on y index
     * @param yFactor multiplier to use to shift in y direction based on x index
     * @return the resulting picture
     */
    ...
    /**
     * Method to try different composite rules in a picture with a gradient
     * painted rectangle and a texture painted ellipse
     */
    ...
    /**
     * Method to copy the passed picture into the current picture at the given x
     * and y position in the current picture
     *
    ...
    /**
     * Method to overlap one picture with another horizontally on top of the
     * current picture. First the part of the first picture before the overlap
     * will be displayed, next to that will be the overlapping region up to the
     * end of the first picture, after that is the remainder of the second
     * picture
     *
    ...
    /**
     * Method to overlap one picture with another vertically on top of the
     * current picture. First the part of the first picture before the overlap
     * will be displayed, below that will be the overlapping region up to the
     * end of the first picture, after that is the remainder of the second
     * picture
     *
     * @param p1 the first picture to display
     * @param p2 the second picture to display
     * @param startOverlap the y position where the overlap begins
     */
    ...
    /**
     * Method to draw a town with 4 streets with houses of different sizes
     */
     ...
     * @param houseHeight the height of the rectangle part of the house (roof is
     * taller)
     */
     ...
     *
     * @param x the x location for the upper left corner of the rectangle
     * enclosing the sun
     * @param y the y location for the upper left corner of the rectangle
     * enclosing the sun
     * @param width the width of the enclosing rectangle
     * @param height the height of the enclosing rectangle
     */
     ...
     *
     * @return a new picture with the image clipped to a triangle
     */
    ...
    /**
     * Method to create a new picture from this picture that shows the part of
     * the current picture that is inside a text message
     *
    ...
    /**
     * Method to blend two pictures, the current picture object and the passed
     * sourcePict object. The amount of the source picture to use is given in
     * sourceAlpha. The amount of the current picture to use is (1.0 -
     * sourceAlpha)
     *
     * @param sourcePict the source picture
     * @param sourceAlpha the amount of the source picture to use
     * @return the result of blending the source picture with the current
     * picture object
     */
    ...
    /**
     * Method to copy a picture into the current picture using the passed
     * composite rule
     *
     * @param sourcePict the source picture to copy from
     * @param sourceRect the rectangle that describes what area to copy from
     * @param targetRect the rectangle that describes what area to copy into
    ...
    /**
     * Method to draw glasses on the picture of Barb
     *
    ...
    /**
     * Method to draw a hat on the picture of Barb
     *
     ...
     *
     * @return the picture of Mark with the weight over him
     */
    ...
    /**
     * Method to return a picture created from the file with the passed name
     *
